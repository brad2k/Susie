<?php
$page_type = "product";
$page_sub_type = "sugaring";
$page_title = "Sugaring by Susie Byleckie";
$meta_description = "";
include("header.php");
?>

<h2>Sugaring</h2>

<p>An amazing hair removal alternative to traditional waxing. An ancient Egyptian art, sugaring provides longer lasting results without skin damage. All natural ingredients.</p>

<table>
	<tbody>
		<tr>
			<td>Eyebrow Shaping</td>
			<td class="price">$20</td>
		</tr>
		<tr>
			<td>Upper Lip</td>
			<td class="price">$16</td>
		</tr>
		<tr>
			<td>Sideburns</td>
			<td class="price">$18</td>
		</tr>
		<tr>
			<td>Ears</td>
			<td class="price">$20</td>
		</tr>
		<tr>
			<td>Full Face (brows not included)</td>
			<td class="price">$45</td>
		</tr>
		<tr>
			<td>Hands</td>
			<td class="price">$22</td>
		</tr>
		<tr>
			<td>Arms (Hands not included)</td>
			<td class="price">$38</td>
		</tr>
		<tr>
			<td>Bikini</td>
			<td class="price">$40 and up</td>
		</tr>
		<tr>
			<td>Brazilian</td>
			<td class="price">$65 and up</td>
		</tr>
		<tr>
			<td>Upper legs/Bikini</td>
			<td class="price">$85</td>
		</tr>
		<tr>
			<td>Full legs/Bikini (Feet not included)</td>
			<td class="price">$120</td>
		</tr>
		<tr>
			<td>Abdomen/Chest</td>
			<td class="price">$95</td>
		</tr>
		<tr>
			<td>Half Chest</td>
			<td class="price">$52</td>
		</tr>
		<tr>
			<td>Brow Tint</td>
			<td class="price">$20</td>
		</tr>
		<tr>
			<td>Lash Tint</td>
			<td class="price">$28</td>
		</tr>
		<tr>
			<td>Chin</td>
			<td class="price">$16</td>
		</tr>
		<tr>
			<td>Nose</td>
			<td class="price">$20</td>
		</tr>
		<tr>
			<td>Neck</td>
			<td class="price">$26</td>
		</tr>
		<tr>
			<td>Underarms</td>
			<td class="price">$28</td>
		</tr>
		<tr>
			<td>Breasts</td>
			<td class="price">$20</td>
		</tr>
		<tr>
			<td>Stomach</td>
			<td class="price">$20</td>
		</tr>
		<tr>
			<td>Lower Legs (Feet not included) </td>
			<td class="price">$50</td>
		</tr>
		<tr>
			<td>Feet</td>
			<td class="price">$20</td>
		</tr>
		<tr>
			<td>Feet and Toes</td>
			<td class="price">$28</td>
		</tr>
		<tr>
			<td>Full Back</td>
			<td class="price">$90</td>
		</tr>
		<tr>
			<td>Half Back </td>
			<td class="price">$60</td>
		</tr>
	  </tbody>
</table>

<?php include("footer.php"); ?>